...
namespace Hastane_Otomasyon
{
    public partial class KlinikEkleSil : Form
    {
        ...
        void ekle()
        {
            string isim = Convert.ToString(dataGridView1.Rows[dataGridView1.SelectedRows[0].Index].Cells["klinik_isim"].Value);
            baglanti.Open();
            string komut = "INSERT INTO klinik(klinik_isim) VALUES(@klinik_isim)";
            MySqlCommand ekle = new MySqlCommand(komut, baglanti);
            ekle.Parameters.AddWithValue("@klinik_isim", isim);
        ...
        void sil()
        {
            int id = Convert.ToInt32(dataGridView1.Rows[dataGridView1.SelectedRows[0].Index].Cells["klinik_id"].Value);
            string komut = "DELETE FROM klinik WHERE klinik_id = '" + id + "'";
            baglanti.Open();
